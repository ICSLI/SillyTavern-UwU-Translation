...
const EXTENSION_KEY = 'uwuTranslation';
...
async function initUI() {
  ...
  context.eventSource.on(EventNames.MESSAGE_UPDATED, async (messageId: number) => {
    if (incomingTypes.includes(settings.autoMode)) {
      await generateMessage(messageId, 'incomingMessage');
      context.eventSource.emit('uwu_translation_done', {
        messageId,
        type: 'incomingMessage',
        auto: true,
      });
  ...
  context.eventSource.on(EventNames.IMPERSONATE_READY, async (messageId: number) => {
    if (outgoingTypes.includes(settings.autoMode)) {
      ...
      context.eventSource.emit('uwu_translation_done', eventData);
      context.eventSource.emit('uwu_translation_impersonate', eventData);
    }
  });
...
async function initSettings() {
  ...
  const settingsElement = $('.uwu-translation-settings');
  ...
  const presetSelect = buildPresetSelect('.uwu-translation-settings select.prompt_preset', {
  ...
  // Fix incomplete tooltips for preset buttons
  const createBtn = presetSelect.container.querySelector('.fa-file-circle-plus');
  const renameBtn = presetSelect.container.querySelector('.fa-pencil');
  const deleteBtn = presetSelect.container.querySelector('.fa-trash-can');
  if (createBtn) createBtn.setAttribute('title', 'Create a new preset');
  if (renameBtn) renameBtn.setAttribute('title', 'Rename a preset');
  if (deleteBtn) deleteBtn.setAttribute('title', 'Delete a preset');

  // Profile selection
  context.ConnectionManagerRequestService.handleDropdown(
    '.uwu-translation-settings .profile',
    settings.profile,
    (profile) => {
...
function main() {
  ...
  // Register the uwu-translate slash command
  context.SlashCommandParser.addCommandObject(
    context.SlashCommand.fromProps({
      ...
      helpString: `
      <div>
        Translates a message with the specified ID using UwU Translation.
      ...
      <div>
        ...
        <ul>
          <li>
            <pre><code class="language-stscript">/uwu-translate</code></pre>
            translates the latest message
          </li>
          <li>
            <pre><code class="language-stscript">/uwu-translate -1</code></pre>
            also translates the latest message
          </li>
          <li>
            <pre><code class="language-stscript">/uwu-translate 5</code></pre>
  ...
  context.SlashCommandParser.addCommandObject(
    context.SlashCommand.fromProps({
      ...
      helpString: `
      <div>
        Translates the given text using UwU Translation.
      </div>
      <div>
        <strong>Example:</strong>
        <ul>
          <li>
            <pre><code class="language-stscript">/uwu-translate-text Hello world</code></pre>
            translates "Hello world" to the default target language.
          </li>
          <li>
            <pre><code class="language-stscript">/uwu-translate-text target="es" Hello world</code></pre>
            translates "Hello world" to Spanish.
          </li>
          <li>
            <pre><code class="language-stscript">/uwu-translate-text profile="My-Profile" Hello world</code></pre>
            translates "Hello world" using the "My-Profile" connection profile.
          </li>
          <li>
            <pre><code class="language-stscript">/uwu-translate-text preset="My-Preset" Hello world</code></pre>
